...
The [local cross-encoder reranking comparison](docs/reranking-experiment.md) is evaluator-only and does not change the serving path.
...
# Local CPU reranking experiment: --mode rerank --rerank-depth 20 (or 50).
```
...
Python is the only parsed programming language. YAML/YML, SQL, `.env.example`, and specifically `vite.config.ts` are indexed as bounded config text; `.env`, general TypeScript, Markdown, TOML, binaries, and generated/vendor trees remain unsupported. Re-indexing replaces all rows for a repository name. Serving search remains exact dense vector search; lexical, hybrid RRF, and cross-encoder reranking are explicit experiments only. There is no Git cloning, incremental indexing, background work, authentication, ANN index, agent loop, or deployment automation. The existing frontend is intentionally only a small `/ask` demonstration UI.

The original 40-case evaluation set is DEV. The [audited Evaluation V2 split](docs/evaluation-v2.md) now freezes a 34-case primary holdout and a separate five-case source-coverage challenge. Neither frozen split has been retrieval-scored; the original 50-case [review sheet](docs/holdout-review.md) remains provenance.
